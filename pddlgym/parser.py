...
import re
import os
# print("DEBUG: parser.py file loaded!") 
...
class PDDLProblemParser(PDDLParser):
    ...
    def __init__(self, problem_fname, domain_name, types, predicates, action_names, constants=None):
        ...
        # print(f"DEBUG: Starting PDDLProblemParser.__init__")
        # print(f"DEBUG: problem_fname = {problem_fname}")
        # print(f"DEBUG: File exists: {os.path.exists(problem_fname)}")
        # if os.path.exists(problem_fname):
        #     print(f"DEBUG: File size: {os.path.getsize(problem_fname)} bytes")
      
        # print(f"DEBUG: Trying to read problem file: {problem_fname}")
        ...
        ## Read files.
        # print(f"DEBUG: Reading file: {problem_fname}")
        with open(problem_fname, "r") as f:
            self.problem = f.read().lower()
        
        # print(f"DEBUG: File contents length: {len(self.problem)}")
        # print(f"DEBUG: First 200 chars after reading: {repr(self.problem[:200])}")
        
        self.problem = self._purge_comments(self.problem)
        # print(f"DEBUG: After purging comments: {len(self.problem)} chars")
        # print(f"DEBUG: First 200 chars after purging: {repr(self.problem[:200])}")
    ...
    def _parse_problem(self):
        # print(f"DEBUG: _parse_problem called")
        # print(f"DEBUG: self.problem length: {len(self.problem)}")
        # print(f"DEBUG: self.problem content: {repr(self.problem[:200])}")
      
        #patt = r"\(problem(.*?)\)"
            # Save the full content for detailed inspection
        if len(self.problem) == 0:
            # print("DEBUG: PROBLEM IS EMPTY - STOPPING FOR INSPECTION")
            # print(f"DEBUG: problem_fname was: {self.problem_fname}")
            # print(f"DEBUG: File exists check: {os.path.exists(self.problem_fname)}")
            # This is where your collaborator suggests putting a breakpoint
            #import pdb; pdb.set_trace()  # Breakpoint for empty files
            pass
        patterns = [
        r"\(define\s+\(problem\s+(.*?)\)",  # Modern format: (define (problem name)
    ]
        #patt = r"\((?:define\s+\()?\s*problem\s+([^)\s]+)"
        # self.problem_name = re.search(patt, self.problem).groups()[0].strip()
        self.problem_name = None
        for patt in patterns:
            # print(f"DEBUG: Trying pattern: {patt}")
            match = re.search(patt, self.problem)
            # print(f"DEBUG: Pattern match result: {match}")
            if match is None:
                # print(f"DEBUG: No match found for pattern: {patt}")
                continue
            # print(f"DEBUG: Match groups: {match.groups()}")
            # print(f"DEBUG: Match groups Strip {match.groups()[0].strip()}")
            
            if match:
                self.problem_name = match.groups()[0].strip()
                # print(f"DEBUG: Found problem name: {self.problem_name}")
                break
        # print(f"DEBUG: Found problem name outside: {self.problem_name}")    

        if not self.problem_name:
            # print(f"DEBUG: No problem name found, self.problem = {repr(self.problem)}")
            #breakpoint()
            # print(f"DEBUG self.problem_fname: : {self.problem_fname}")
            raise Exception(f"Could not find problem name in: {self.problem[:200]}")
        
        patt = r"\(:domain(.*?)\)"
        domain_match = re.search(patt, self.problem)
        if domain_match is None:
            # print("DEBUG: Domain pattern failed!")
            raise Exception(f"Could not find domain declaration in: {self.problem[:200]}")
