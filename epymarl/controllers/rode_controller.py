...
class RODEMAC:
    ...
    def select_actions(self, ep_batch, t_ep, t_env, bs=slice(None), test_mode=False):
        ...
        if t_env % 10000 == 0:
            print(f"[RODE] t_env={t_env} Selected roles: {self.selected_roles}")
            print(f"[RODE] Role action spaces:\n{self.role_action_spaces}")
    ...
    def forward(self, ep_batch, t, test_mode=False, t_env=None):
        ...
        if t % self.role_interval == 0:
            ...
            self.selected_roles = self.role_selector.select_role(role_outputs, test_mode=test_mode, t_env=t_env).squeeze()            # [bs * n_agents]
    ...
    def update_role_action_spaces(self):
        ...
        action_repr_array = action_repr.detach().cpu().numpy()

        k_means = KMeans(n_clusters=self.n_clusters, random_state=0).fit(action_repr_array)

        spaces = []
        for cluster_i in range(self.n_clusters):
            space = (k_means.labels_ == cluster_i).astype(float)
            if space.sum() > 0:
                spaces.append(space)

        if len(spaces) == 0:
            spaces = [np.ones(self.n_actions)]
        ...
        self.role_latent = th.matmul(self.role_action_spaces, action_repr) / \
            self.role_action_spaces.sum(dim=-1, keepdim=True)
        self.role_latent = self.role_latent.detach().clone()
        self.action_repr = action_repr.detach().clone()
        print(">>> MARLHospital Role Action Spaces")
