...
class MARLEnv(gym.Env):
    ...
    def __init__(
        ...
        # Handle observation_mode parameter - convert string to enum if needed
        if observation_mode is None:
            self.observation_mode = self.observation_size.LARGE
        elif isinstance(observation_mode, str):
            # Convert string to enum
            mode_mapping = {
                "SMALL": self.observation_size.SMALL,
                "MEDIUM": self.observation_size.MEDIUM,
                "LARGE": self.observation_size.LARGE
            }
            self.observation_mode = mode_mapping.get(observation_mode.upper(), self.observation_size.LARGE)
        else:
        ...
        ) = self._get_observation_space(self.observation_mode)
    ...
    def step(self, expanded_truths, valid_actions):
        ...
        # Update the state dimensions based on the new observation space
        new_state_size = len(self.shortened_expanded_truths + self.shortened_action_truths[0])
        self.state = np.zeros((self.n_agents, new_state_size))
        self.state_names = [[]] * self.n_agents
        ...
        # Update the observation space to match the new state size
        self.observation_space = spaces.MultiBinary(len(self.state[0]))
        
        # Debug print to verify the update
        # print(f"DEBUG: Updated observation space to: {self.observation_space}")
        # print(f"DEBUG: State shape: {self.state.shape}")
        # self.print_state()

    def _get_observation_space(self, mode):
        """
